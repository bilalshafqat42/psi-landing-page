...
const GallerySection = () => {
  ...
  return (
    <section className="min-h-screen bg-white text-gray-900 px-6 py-20 flex items-center justify-center gap-12 relative font-[Poppins]">
      <div className="w-full max-w-[1440px] flex flex-col md:flex-row items-start gap-12 mx-auto">
        {/* Left Content */}
        <div className="md:w-2/5 z-10">
          <h2 className="text-4xl font-bold leading-tight mb-4">Welcome to Online Book Store</h2>
          ...
          <ul className="flex gap-8 mb-6">
            <li>
              <h3 className="text-2xl font-semibold"><i className="fa-solid fa-book mr-2"></i>68+k</h3>
              <span className="text-xs text-gray-600 block mt-1">book collections</span>
            </li>
            <li>
              <h3 className="text-2xl font-semibold"><i className="fa-solid fa-user mr-2"></i>25,634</h3>
              ...
              >
                ...
                {(slide.title || slide.subtitle) && (
                  <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 w-[90%] text-center text-white">
                    {slide.title && <h3 className="text-lg font-bold mb-1">{slide.title}</h3>}
          ...
          >
            {slides.map((slide, index) => (
              <SwiperSlide key={index} className="flex items-center justify-center">
                <img src={slide.src} alt={slide.title || 'Preview'} className="max-h-[90vh] object-contain mx-auto" />
              </SwiperSlide>
            ))}
          </Swiper>
          <button className="absolute top-6 right-6 text-white text-2xl z-50" onClick={() => setModalImage(null)}>&times;</button>
        </div>
      )}

      {/* Decorative Circle */}
      <div className="absolute -bottom-20 -left-32 w-[40vw] h-[40vw] max-w-[400px] max-h-[400px] bg-black opacity-70 rounded-full z-0"></div>
    </section>
  );
};
